...
//creates a new link list in three ways
//returns created link list pointer
node *createLinklist(){
    int i=1, elem;
    int mode  = 1;
    int count = 0;
...
//returns maximum number in from linklist
...
//Insert new node after given value
//gives error messages if given value
//is not present in linklist
void insertAfter(node *start, int value, int newValue){
...
//Insert new node before entered node
...
//Delete a node after entered node
//returns pointer to node *
void deleteAfter(node *start, int value){
    node *temp;
    node *toDelete;
...
//reverses original linklist and returns new start
node *reverse(node *start){
    node *newStart = NULL;
    node *previous = NULL;
    node *current  = NULL;
...
//reverses original linklist by using recurssion
void reverseByRec(node *temp, node **head){
    node *next = NULL;
    if (temp->ptr != NULL){
        next = temp->ptr;
        reverseByRec(next, head);
        next->ptr = temp;
        temp->ptr = NULL;
    }
    else {
...
//returns new linklist which is reversed
node *createReversed(node *start){
    node *newNode = NULL;
    node *previous = NULL;
    node *temp = NULL;
...
//merge two decreasing linklist and returns pointer to new linklist
node *mergeLinklist(node *first, node *second){
    if (first == NULL){
        return second;
    }
