 ...
 * Desciption : to solve single digit 
 *              infix to postfix equations
 * ******************************/
#pragma once
...
char *makeInfix();
void solveInfixFile(char *filename);
...
void solveInfixFile(char *filename)
{
	///solve infix equation from file
	printf("=============================================\n");
	printf("solving infix equation from file %s\n", filename);
	printf("=============================================\n");
	FILE *infixFile = NULL;
	int i = 0, ans, len, inflen, postlen;
	char infix[MAX], postfix[MAX];
	infixFile = fopen(filename, "r");
	fscanf(infixFile, "%d", &len);
	while (i < len)
	{
		fscanf(infixFile, "%s", infix);
		printf("\ninfix   : %s\n", infix);
		inflen = strlen(infix);
		if (validateInfix(infix, inflen))
		{
			convertInfixToPostfix(infix, postfix, inflen);
			postlen = strlen(postfix);
			ans = solvePostfix(postfix, postlen);
			printf("postfix : %s\n", postfix);
			printf("solu    : %d\n", ans);
		}
		i++;
